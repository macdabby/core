		...
		initialize: function($searchBox, $searchResults) {
			...
			function addResults(results) {
				...
				if (count === 0) {
					...
					$status.append('<h2>' + t('core', 'No search results elsewhere') + '</h2>');
				} else {
					$status.removeClass('emptycontent').addClass('status');
					$status.text(n('core', '{count} search result elsewhere', '{count} search results elsewhere', count, {count:count}));
				}
			}
